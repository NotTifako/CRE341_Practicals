...
public class MapGeneratorMine : MonoBehaviour {
	...
	void ProcessMap() {
        ...
        roomsList = survivingRooms;
	}

    void SpawnBlocks(List<Room> rooms)
    {
        foreach(Room room in rooms)
        {
            if(room.roomSize < 150)
            {
                Debug.Log("Room smaller than 150");
                
                int randomIndex = Random.Range(0,room.roomSize);

                Coord thingCoord = new Coord(room.tiles[randomIndex].tileX, room.tiles[randomIndex].tileY);
                Vector3 location = CoordToWorldPoint(thingCoord);
                
                GameObject thing = Instantiate(thingPrefab, new Vector3(location.x, -2.41f, location.z),Quaternion.identity);
            }
            else if(room.roomSize >= 150 && room.roomSize < 250)
            {
                Debug.Log("Room between 150 and 250");
                int randomIndex = GetRandomRoomTile(room);

                Coord wallCoord = new Coord(room.edgeTiles[randomIndex].tileX, room.edgeTiles[randomIndex].tileY);
                Vector3 location = CoordToWorldPoint(wallCoord);
    ...
    int GetRandomRoomTile(Room room)
    {
        int attempts = 0;

        for(int i = 0; i < maxAttempts; i++)
        {
            int randomIndex = Random.Range(0,room.edgeTiles.Count);
            Vector3 origin = new Vector3(room.tiles[randomIndex].tileX, raycastHeight, room.tiles[randomIndex].tileY);
            ...
            if (Physics.Raycast(origin, Vector3.down, out RaycastHit hit, Mathf.Infinity))
            {
                ...
                if (hit.collider.CompareTag("Ground") || hit.collider.name == "NavMesh")
                {
                    i = maxAttempts;
                    return randomIndex; 
                }
    ...
    public Vector3 GetRandomGroundPoint()
    {
		...
		for (int i = 0; i < maxAttempts; i++)
        {
            ...
            float randZ = Random.Range(groundBounds.min.z, groundBounds.max.z/2);//--------------------------------------------------------------------------------------------------
